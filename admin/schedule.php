...
// Get court ID from session
$court_id = $_SESSION['court_id'] ?? $_SESSION['admin_court_id'] ?? null;

if (!$court_id) {
    header('Location: dashboard.php?error=court_not_found');
    exit();
}
...
<head>
    ...
    <title>Weekly Schedule - <?php echo htmlspecialchars($court['court_name']); ?></title>
...
<body class="schedule-page">
    ...
    <div class="schedule-container">
        ...
        <div class="navigation-buttons">
            <a href="dashboard.php" class="btn btn-control" title="Back to Dashboard">
                ◀ Back
            </a>
        ...
        <div class="week-navigation">
            <div class="week-controls">
                <a href="?week=<?php echo $prev_week; ?>" class="nav-button">
                    ← Previous Week
                </a>
                <a href="?week=<?php echo date('Y-m-d', strtotime('monday this week')); ?>" class="nav-button secondary">
                    This Week
                </a>
                <a href="?week=<?php echo $next_week; ?>" class="nav-button">
                    Next Week →
                </a>
            </div>
        ...
        <div class="schedule-grid" id="scheduleGrid">
            ...
            try {
                ...
                // Time slots (7 AM to 12 AM / Midnight)
                $time_slots = [];
                for ($hour = 7; $hour <= 23; $hour++) {
                    $time_slots[] = $hour;
                }
                $time_slots[] = 0;  // Add midnight (12 AM)
                ...
                foreach ($bookings as $booking) {
                    ...
                    // Handle midnight booking (0 = 12 AM)
                    ...
                    for ($h = $start_hour; $h < $end_hour; $h++) {
                        $hour_key = ($h == 24) ? 0 : $h;  // Convert 24 to 0 for midnight
                ...
                foreach ($time_slots as $hour) {
                    ...
                    if ($hour == 0) {
                        ...
                        $time_display = ($hour - 12) . ' PM';
                    } else {
                        $time_display = $hour . ' AM';
                    }
                    ...
                    foreach ($week_dates as $date) {
                        ...
                        if ($booking_data) {
                            ...
                            echo '<div class="schedule-cell available" title="Available - No bookings"></div>';
                        }
                    }
                }
        ...
        <div class="legend">
            ...
            <div class="legend-item">
                <div class="legend-color" style="background: #515860;"></div>
                <span>Declined</span>
            </div>
        </div>

        <!-- Week Summary -->
        <?php if (isset($week_stats)): ?>
        <div class="week-summary">
            <h3>📊 Week Summary</h3>
            <div style="display: flex; justify-content: center; gap: 2rem; flex-wrap: wrap; margin-top: 1rem;">
                <div>
                    <strong style="color: var(--accent-blue);"><?php echo $week_stats['total_bookings']; ?></strong>
                    <br><small>Total Bookings</small>
                </div>
                <div>
                    <strong style="color: var(--accent-red);"><?php echo $week_stats['confirmed']; ?></strong>
                    <br><small>Confirmed</small>
                </div>
                <div>
                    <strong style="color: var(--accent-yellow);"><?php echo $week_stats['pending']; ?></strong>
                    <br><small>Pending</small>
                </div>
                <div>
                    <strong style="color: var(--text-secondary);"><?php echo $week_stats['declined']; ?></strong>
                    <br><small>Declined</small>
                </div>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div style="text-align: center; margin-top: 2rem; color: var(--text-secondary); margin-bottom: 4rem;">
            <p><strong>Court Operation Hours:</strong> 7:00 AM - 12:00 AM (Midnight)</p>
            <p><strong>Hover over colored cells</strong> to see booking details and payment amounts</p>
        </div>
    </div>
    ...
    <script>
        // Jump to specific week function
        ...
        cells.forEach(cell => {
            cell.addEventListener('mouseenter', function(e) {
                ...
                tooltip.innerHTML = `
                    ...
                    <div class="tooltip-amount">Payment: ₱${data.amount}</div>
                `;
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'ArrowLeft') {
                document.querySelector('[href*="week=<?php echo $prev_week; ?>"]').click();
            } else if (e.key === 'ArrowRight') {
                document.querySelector('[href*="week=<?php echo $next_week; ?>"]').click();
            }
        });
    </script>
</body>
